    ...
    def save_yaml(self, odo_data):
        ...
        with open(self.INPUT_BAG_PATH + '_db_estimation.yaml', mode=mode) as odometry_yaml:
            ...
            for pose in odo_data:
                ...
                row.append(str("%.8f" % z))
                x = message.transform.rotation.x
                row.append(str("%.8f" % x))
                y = message.transform.rotation.y
                row.append(str("%.8f" % y))
                z = message.transform.rotation.z
                row.append(str("%.8f" % z))
                w = message.transform.rotation.w
                row.append(str("%.8f" % w))

                data["odometry_data"][message.header.stamp] = row

            # self.logger.info(row)
            # self.logger.info(len(data["odometry_data"]))
            time_stamps = data["odometry_data"].keys()
            # self.logger.info(len(data["odometry_data"].keys()))
            min_time_stamp = min(time_stamps)
            flo_min_time = float(min_time_stamp.secs)
            flo_min_time_n = float(min_time_stamp.nsecs)
            min_time_stamp_float = flo_min_time + flo_min_time_n * 10**(-9)
            data["begin_time_stamp"] = min_time_stamp_float

            keys = {}
            for key, _ in data["odometry_data"].iteritems():
                # self.logger.info((key))
                new_key = key - min_time_stamp
                # self.logger.info(new_key)
                flo_time = float(new_key.secs)
                flo_time_n = float(new_key.nsecs)
                new_key_float = flo_time + flo_time_n * 10**(-9)
                # self.logger.info(type(new_key_float))
                # self.logger.info(new_key_float)
                keys[key] = "%08.4f" % new_key_float

            for key, new_key in keys.iteritems():
                data["odometry_data"][new_key] = data["odometry_data"].pop(
                    key)

            yaml.dump(data, odometry_yaml)
